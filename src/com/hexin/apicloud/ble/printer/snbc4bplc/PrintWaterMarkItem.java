...
public class PrintWaterMarkItem implements IPrintTemplateItem {
	...
	private void printTextAutoLine(ILabelEdit labelEdit,String content,int width,int startX,int startY,int fontSize) throws Exception{
	    ...
	    for(int i = 0,j = content.length();i < j;i++){ 
	    	...
	    	// -5 防止边界出现
	        if(lineWidth > width-5){
	        	printText(labelEdit,startX,startY,content.substring(lastSubStrIndex,i),fontSize);
	            // 行间距 5
	        	startY += fontSize + 5;
	            lineWidth = 0;
	            lastSubStrIndex = i;
	        ...
	        if(i == content.length()-1){
	        	printText(labelEdit,startX,startY,content.substring(lastSubStrIndex,i+1),fontSize);
	        }
	    }
	}
	
	private void printText(ILabelEdit labelEdit,int startX,int startY,String content,int fontsize) throws Exception{
		switch (fontsize){
			case 16:
				labelEdit.printText(startX,startY, "55", content, Rotation.Rotation0, 0,0,0);
				break;
			case 24:
				labelEdit.printText(startX,startY, "8", content, Rotation.Rotation0, 0,0,0);
				break;
			case 32:
				labelEdit.printText(startX,startY, "55", content, Rotation.Rotation0, 0,3,0);
				break;
			case 48:
				labelEdit.printText(startX,startY, "8", content, Rotation.Rotation0, 0,3,0);
				break;
			default:
				labelEdit.printText(startX,startY, "55", content, Rotation.Rotation0, 0,6,0);
				break;
		}
	}
	...
	public enum FontSizeEnum {
		...
		HEIGHT16(16,7),HEIGHT24(24,10),HEIGHT32(32,15),HEIGHT48(48,20),HEIGHT64(64,96);
